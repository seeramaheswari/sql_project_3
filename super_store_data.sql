...
select monthname(`Order Date`) as Month,Region,Category,
round(sum(sales),2) as Total_sales
from train
group by monthname(`Order Date`) ,Region, Category
order by monthname(`Order Date`),Total_sales desc;
...
-- write sql to display the regional sale_count performance and growth over month
with yearly_sales as(
select year(`order date`) as order_year,count(*) as sales_count
 from train group by year(`Order Date`)
),
growth_cal as(
select order_year,sales_count,lag(sales_count) over(
order by order_year) as prev_sales_count from yearly_sales
)
select *,((sales_count-prev_sales_count)/prev_sales_count)* 100 as growth_prct from growth_cal;
